...
int dumpToken() {
  ...
  while (lexer.getNextToken() != tok_eof) {
    if(lexer.getCurToken() == pony::tok_identifier) {
      token_vector.push_back(pony::tok_identifier);
      str_vector.push_back(lexer.getId().str());
      index_vector.push_back(str_vector.size()-1);
    }
    else if(lexer.getCurToken() == pony::tok_number) {
      token_vector.push_back(pony::tok_number);
      num_vector.push_back(lexer.getValue());
      index_vector.push_back(num_vector.size()-1);
    }
    else if(lexer.getCurToken() == pony::tok_def || lexer.getCurToken() == pony::tok_var || lexer.getCurToken() == pony::tok_return || lexer.getCurToken() == ';' || lexer.getCurToken() == '(' || lexer.getCurToken() == ')' || lexer.getCurToken() == '[' || lexer.getCurToken() == ']' || lexer.getCurToken() == '{'|| lexer.getCurToken() == '}') {
      token_vector.push_back(lexer.getCurToken());
      index_vector.push_back(-1);
    }
    else if(lexer.getCurToken() == 0) invalidToken = true;
  } 

  // 如果有非法token，不输出token
  if (invalidToken) return 0;

  // 输出token
  for(int i = 0; i < token_vector.size(); ++i) {
    if (token_vector[i] == pony::tok_identifier) 
      std::cout << str_vector[index_vector[i]] << ' ';
    else if (token_vector[i] == pony::tok_number)
      std::cout << num_vector[index_vector[i]] << ' ';
    else if (token_vector[i] == pony::tok_def)
      std::cout << "def ";
    else if (token_vector[i] == pony::tok_var)
      std::cout << "var ";
    else if (token_vector[i] == pony::tok_return)
      std :: cout << "return ";
    else if (token_vector[i] == pony::tok_eof)
      continue;
    else if (token_vector[i] == ';' || token_vector[i] == '(' || token_vector[i] == ')'|| token_vector[i] == '[' || token_vector[i] == ']' || token_vector[i] == '{' || token_vector[i] == '}')
      std::cout << char(token_vector[i]) << ' ';
  }
  std::cout << std::endl;
